...
const GFONTS_ORIGIN = { name: 'googleFontsCssOrigin', type: 'string', def: "'https://fonts.googleapis.com'", desc: 'HTTP(S) origin for a Google Fonts-compatible CSS service. This is not a general-purpose webfont resolver: it only relocates the built-in Google Fonts stylesheet requests, preserving their paths and family queries. Relative font-file URLs from the returned CSS resolve against the final stylesheet URL after redirects. Used only when useGoogleFonts is true; a service failure falls back to system fonts without retrying the public endpoint.', emphasis: 'Used only when useGoogleFonts is true' };
