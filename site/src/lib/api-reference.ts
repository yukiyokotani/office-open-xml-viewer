  ...
  docx: [
    ...
    },
    {
      ...
      methods: [
        ...
        { sig: 'setLayoutView(view?: { showTrackedChanges?, currentDate? }): Promise<void>', desc: 'Select the layout variant the geometry accessors (pageCount, pageSize, bookmark lookup) read from, matching the variant being rendered. Viewers call this when the markup view toggles. In `mode: "worker"` the variant is repaginated in the worker, and the view and its metadata are installed together when that lands. Await it before reading geometry.', emphasis: 'Await it before reading geometry.' },
    ...
    },
    {
      ...
      methods: [
        ...
        { sig: 'setShowTrackedChanges(value: boolean): Promise<void>', desc: 'Switch between the final view (false, default) and the tracked-change markup view (true) at runtime. Resolves once the switch is on screen: every mounted page re-renders against the selected layout variant and the page count follows the new pagination. The previous variant stays painted — and measured — until then.' },
        ...findMethods('DocxMatchLocation'),
        { sig: 'setScale(scale: number): void', desc: 'Set the absolute zoom scale at runtime (clamped to zoomMin/zoomMax). Flicker-free. View-only.' },
        { sig: 'relayout(): void', desc: 'Force a re-fit + re-mount of the visible window. Called automatically after load / resize / zoom; use it when the container resizes in a way a ResizeObserver cannot observe (e.g. a late web-font load). Idempotent.' },
        { sig: 'get pageCount(): number', desc: 'Total pages (0 until loaded).' },
