...
export const formatRenderModeGuidance: Readonly<Record<'docx' | 'xlsx' | 'pptx', string>> = {
  docx: 'DOCX keeps page navigation, virtualized scrolling, selection, find, hyperlinks, equations, 3-D charts and Region Maps in both modes. Worker mode moves pagination and page paint away from the UI thread. Documents that require DOM OpenType vertical-glyph selection automatically use main mode; read the loaded document\'s mode to observe that fallback.',
  xlsx: 'XLSX keeps sheet tabs, frozen panes, scrolling, selection, find, hyperlinks, equations, 3-D charts and Region Maps in both modes. Worker mode paints each requested sheet viewport away from the UI thread.',
  pptx: 'PPTX keeps slide navigation, virtualized scrolling, selection, find, hyperlinks, media playback, equations, 3-D charts and Region Maps in both modes. Worker mode moves slide paint away from the UI thread; media controls and overlays remain interactive in the Viewer.',
};
  ...
  pptx: [
    ...
    },
    {
      ...
      options: [
        ...
        { name: 'onSelectionContextChange', type: '(context: PptxSelectionContext | null) => void', desc: 'Receive bounded detached text or element context for external AI/MCP integrations. This callback does not enable element hit-testing.' },
        CONTEXT_MENU('PptxSelectionContext'),
        FIND_HIGHLIGHT_COLORS,
      ...
      methods: [
        ...
        { sig: 'getSelectionContext(options?: PptxSelectionContextOptions): PptxSelectionContext | null', desc: 'Return the current mounted text selection or clicked-element context.' },
        RESOURCE_METRICS_METHOD,
  ...
  docx: [
    ...
    },
    {
      ...
      options: [
        ...
        { name: 'onSelectionContextChange', type: '(context: DocxSelectionContext | null) => void', desc: 'Receive bounded detached text or element context for external AI/MCP integrations. This callback does not enable element hit-testing.' },
        CONTEXT_MENU('DocxSelectionContext'),
        FIND_HIGHLIGHT_COLORS,
        ON_HYPERLINK_CLICK,
        ENABLE_HYPERLINKS,
      ...
      methods: [
        ...
        { sig: 'getSelectionContext(options?: DocxSelectionContextOptions): DocxSelectionContext | null', desc: 'Return the current mounted text selection or clicked-element context.' },
        RESOURCE_METRICS_METHOD,
        { sig: 'destroy(): void', desc: 'Tear down the DOM subtree. Destroys a self-loaded engine; a borrowed one is left intact.' },
      ],
    },
  ...
  xlsx: [
    {
      ...
      options: [
        ...
        { name: 'onSelectionContextChange', type: '(context: XlsxSelectionContext | null) => void', desc: 'Receive bounded detached range or element context for read-only AI/MCP handoff. This callback does not enable element hit-testing; rapid changes are coalesced per animation frame.' },
      ...
      methods: [
        ...
        { sig: 'getSelectionContext(options?: { maxCells?: number; maxTextCharacters?: number }): XlsxSelectionContext | null', desc: 'Return bounded `{ kind: "range" }` cell content or `{ kind: "element" }` clicked-object context for read-only AI/MCP handoff.' },
    ...
    },
    {
      ...
      options: [
        ...
        { name: 'onSelectionContextChange', type: '(context: XlsxSelectionContext | null) => void', desc: 'Receive bounded detached range or element context, coalesced per animation frame. This callback does not enable element hit-testing.' },
        CONTEXT_MENU('XlsxSelectionContext'),
        VIEWER_ON_ERROR,
      ],
      methods: [
        ...
        { sig: 'getSelectionContext(options?: { maxCells?: number; maxTextCharacters?: number }): XlsxSelectionContext | null', desc: 'Return bounded range content or clicked chart, picture, or shape context for AI/MCP use.' },
