...
const GFONTS = { name: 'useGoogleFonts', type: 'boolean', def: 'false', desc: 'Use the built-in GoogleFontsProvider for metric-compatible webfonts and non-Latin script fallbacks (Noto Arabic / CJK KR·SC·TC·JP / Cyrillic / Hebrew / Thai / Devanagari). It uses the same main-thread and worker pipeline as fontProvider. Off by default for privacy.', emphasis: 'Off by default for privacy.' };
