...
const meta: Meta<Args> = {
  title: 'PptxViewer/Markdown',
};
export default meta;

type Story = StoryObj<Args>;

// Cache the wasm fetch+init across multiple files in the same session so the
// .wasm download only happens on the first conversion.
...
const DEMO_URL = `${import.meta.env.BASE_URL}pptx/demo/sample-1.pptx`;
...
export const Markdown: Story = {
  ...
  render() {
    ...
    pre.textContent = `Pick a .pptx file or click "Load demo".`;
    root.append(pre);
