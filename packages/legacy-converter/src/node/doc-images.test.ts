// An inline binary Word PNG picture (MS-DOC PICFAndOfficeArtData, MS-ODRAW
// cropFromLeft) through the ordinary DOCX renderer.
...
it.skipIf(!skia).each([0, 16384])('renders an inline PNG with fixed-point left crop %i', async crop => {
  ...
  try {
    ...
    // A 16384/65536 left crop removes half of the red half.
    expect(red / blue).toBeCloseTo(crop === 0 ? 1 : 0.5, 1);
