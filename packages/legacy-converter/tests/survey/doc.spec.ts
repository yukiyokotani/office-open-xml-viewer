// Local-only legacy DOC survey against Office-exported PDFs, run from the
// legacy converter package (see playwright.config.ts). The page is the
// DOCX package's own VRT fixture on its dev server; this package supplies
// the direct DOC source, so no OOXML package test imports the converter.
