...
describe('NativeTextSelectionDragRetainer', () => {
  ...
  function fixture() {
    ...
    const selection = {
      anchorNode: null as FakeNode | null,
      anchorOffset: 0,
      focusNode: null as FakeNode | null,
      focusOffset: 0,
      isCollapsed: true,
      rangeCount: 1,
    ...
    const ownerDocument = Object.assign(documentTarget, {
    ...
    const runA = { parentNode: root, dataset: { ooxmlSelectionRun: 'docx' } } as FakeNode;
    const runB = { parentNode: root, dataset: { ooxmlSelectionRun: 'docx' } } as FakeNode;
    const textA = { parentNode: runA } as FakeNode;
    const textB = { parentNode: runB } as FakeNode;
    const gap = { parentNode: root } as FakeNode;
    return { documentTarget, rootTarget, root, runA, runB, textA, textB, gap, selection };
  }

  it('retains the last valid directional range while a text drag crosses a gap', () => {
    const { documentTarget, rootTarget, root, runA, textA, textB, gap, selection } = fixture();
    const retainer = new NativeTextSelectionDragRetainer(root as unknown as HTMLElement);

    rootTarget.dispatch('pointerdown', { button: 0, isPrimary: true, pointerId: 7, target: runA });
    expect(documentTarget.listenerCount('selectionchange')).toBe(1);

    // Backward selection: preserving anchor/focus, rather than only a sorted
    // Range, is required so re-entering selectable text continues naturally.
    ...
    documentTarget.dispatch('pointermove', { pointerId: 7, target: gap });
    documentTarget.dispatch('selectionchange');

    expect(selection.setBaseAndExtent).toHaveBeenCalledWith(textB, 3, textA, 1);

    documentTarget.dispatch('pointerup', { pointerId: 7 });
    const restoreCount = selection.setBaseAndExtent.mock.calls.length;
    Object.assign(selection, { anchorNode: gap, focusNode: gap, isCollapsed: true });
    documentTarget.dispatch('selectionchange');
    expect(selection.setBaseAndExtent).toHaveBeenCalledTimes(restoreCount);
    expect(documentTarget.listenerCount('selectionchange')).toBe(0);

    retainer.destroy();
  });

  it('does not retain selections for drags that start outside a tagged run', () => {
    const { documentTarget, rootTarget, root, gap, selection } = fixture();
    const retainer = new NativeTextSelectionDragRetainer(root as unknown as HTMLElement);

    rootTarget.dispatch('pointerdown', { button: 0, isPrimary: true, pointerId: 1, target: gap });
    expect(documentTarget.listenerCount('selectionchange')).toBe(0);
  ...
  it('captures the post-pointermove selection when selectionchange is coalesced', async () => {
    const { documentTarget, rootTarget, root, runA, runB, textA, textB, gap, selection } = fixture();
    const retainer = new NativeTextSelectionDragRetainer(root as unknown as HTMLElement);

    rootTarget.dispatch('pointerdown', { button: 0, isPrimary: true, pointerId: 9, target: runA });
    documentTarget.dispatch('pointermove', { pointerId: 9, target: runB });
    Object.assign(selection, {
      anchorNode: textA,
      anchorOffset: 1,
      focusNode: textB,
    ...
    documentTarget.dispatch('pointermove', { pointerId: 9, target: gap });
    Object.assign(selection, {
      anchorNode: gap,
      anchorOffset: 0,
      focusNode: gap,
    ...
    expect(selection.setBaseAndExtent).toHaveBeenCalledWith(textA, 1, textB, 2);
    retainer.destroy();
  });

  it('rejects a focus endpoint that snaps away from the run under the pointer', () => {
    const { documentTarget, rootTarget, root, runA, textA, textB, selection } = fixture();
    const retainer = new NativeTextSelectionDragRetainer(root as unknown as HTMLElement);

    rootTarget.dispatch('pointerdown', { button: 0, isPrimary: true, pointerId: 11, target: runA });
    Object.assign(selection, {
      anchorNode: textA,
      anchorOffset: 1,
      focusNode: textA,
    ...
    // Sparse, absolutely positioned overlays can make the browser snap the
    // focus into an unrelated DOM run even though the pointer remains on A.
    Object.assign(selection, {
      anchorNode: textA,
      anchorOffset: 1,
      focusNode: textB,
    ...
    documentTarget.dispatch('pointermove', { pointerId: 11, target: runA });
    documentTarget.dispatch('selectionchange');

    expect(selection.setBaseAndExtent).toHaveBeenCalledWith(textA, 1, textA, 3);
    retainer.destroy();
  });
});
