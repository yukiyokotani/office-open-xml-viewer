...
interface DirectionalSelectionSnapshot {
...
/**
 * Keep a native text drag stable while its pointer crosses the sparse blank
 * regions of a Canvas text overlay.
 *
 * DOCX and PPTX expose selectable glyphs as absolutely positioned tagged runs
 * over a bitmap. Outside those runs the browser hit-tests the bitmap or a
 * positioning frame and may collapse the Selection or snap its focus to an
 * unrelated run. This guard records only browser-produced, run-to-run
 * directional endpoints. While the active pointer is outside every tagged run,
 * it restores that last valid state; it performs no coordinate search and does
 * not invent a nearest character or document-specific selection rule.
 *
 * Document listeners exist only for an active primary-pointer drag that began
 * on `[data-ooxml-selection-run]`, so idle viewers pay no pointer-move cost.
 */
export class NativeTextSelectionDragRetainer {
  private readonly ownerDocument: Document;
  private activePointerId: number | null = null;
  private pointerRun: Node | null = null;
  private snapshot: DirectionalSelectionSnapshot | null = null;
  ...
  private readonly onPointerDown = (event: PointerEvent): void => {
    const pointerRun = this.findTaggedRunTarget(event.target);
    if (this.destroyed || this.activePointerId !== null || event.button !== 0 ||
        event.isPrimary === false || !pointerRun) return;
    this.activePointerId = event.pointerId;
    this.pointerRun = pointerRun;
    this.snapshot = null;
  ...
  private readonly onPointerMove = (event: PointerEvent): void => {
    ...
    this.pointerRun = this.findTaggedRunTarget(event.target);
    // Native Selection updates are a pointer-move default action. Re-check in a
    // microtask after that action even on engines that coalesce selectionchange.
  ...
  private readonly onPointerCancel = (event: PointerEvent): void => {
    if (this.matchesActivePointer(event)) this.endDrag();
  };
  ...
  private syncSelection(): void {
    const selection = this.ownerDocument.getSelection?.() ?? null;
    if (this.pointerRun && this.isRunBoundedSelection(selection) &&
        this.findTaggedRunTarget(selection.focusNode) === this.pointerRun) {
      this.captureSnapshot(selection);
    } else if (this.snapshot) {
      this.restoreSnapshot(selection);
    }
  }
  ...
  private findTaggedRunTarget(target: EventTarget | null): Node | null {
    let node = target as (Node & { dataset?: DOMStringMap }) | null;
    while (node && this.root.contains(node)) {
      if (node.dataset?.ooxmlSelectionRun !== undefined) return node;
      if (node === this.root) break;
      node = node.parentNode as (Node & { dataset?: DOMStringMap }) | null;
    }
    return null;
  }

  private isRunBoundedSelection(selection: Selection | null): selection is Selection & {
    anchorNode: Node;
    focusNode: Node;
  } {
    return !!selection && !selection.isCollapsed && selection.rangeCount > 0 &&
      selection.anchorNode !== null && selection.focusNode !== null &&
      this.findTaggedRunTarget(selection.anchorNode) !== null &&
      this.findTaggedRunTarget(selection.focusNode) !== null;
  }

  private captureSnapshot(selection = this.ownerDocument.getSelection?.() ?? null): void {
    if (!this.isRunBoundedSelection(selection)) return;
    this.snapshot = {
  ...
  private restoreSnapshot(selection = this.ownerDocument.getSelection?.() ?? null): void {
    const snapshot = this.snapshot;
    if (!selection || !snapshot) return;
    if (!this.root.contains(snapshot.anchorNode) || !this.root.contains(snapshot.focusNode)) {
      this.snapshot = null;
      return;
    }
    if (selection.anchorNode === snapshot.anchorNode &&
        selection.anchorOffset === snapshot.anchorOffset &&
        selection.focusNode === snapshot.focusNode &&
        selection.focusOffset === snapshot.focusOffset) return;
