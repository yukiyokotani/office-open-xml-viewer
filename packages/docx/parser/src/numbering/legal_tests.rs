...
fn legal_numbering_uses_common_boolean_semantics_in_both_namespaces() {
    for ns in [TRANSITIONAL, STRICT] {
        for (property, legal) in [
            ...
            assert_eq!(
                map.get_level(2, 2).expect("test fixture").format,
                "decimalZero"
            );
        }
    }
...
fn legal_numbering_preserves_counter_progression_and_literal_text() {
    ...
    let definition = &mut legal.abstract_nums.get_mut(&1).expect("test fixture")[2];
...
fn legal_numbering_uses_normative_decimal_even_for_none_and_zero() {
    ...
    let mut legal = map(TRANSITIONAL, "<w:isLgl/>", None);
    legal.abstract_nums.get_mut(&1).expect("test fixture")[0].format = "none".into();
...
fn legal_numbering_reaches_both_document_parse_routes() {
    ...
    crate::parser::write_test_content_types(&mut zip);
    for (name, content) in [
        ...
        zip.start_file(name, SimpleFileOptions::default())
            .expect("test fixture");
        zip.write_all(content.as_bytes()).expect("test fixture");
    }
    let bytes = zip.finish().expect("test fixture").into_inner();
    for parse in [
        crate::parser::parse_from_bytes_with_limits,
        crate::parser::parse_from_bytes_streamed_with_limits,
    ] {
        let value = serde_json::to_value(parse(&bytes, None, None, "parse").expect("test fixture"))
            .expect("test fixture");
        let texts: Vec<&str> = value["body"]
            .as_array()
            .expect("test fixture")
            .iter()
            .map(|p| p["numbering"]["text"].as_str().expect("test fixture"))
            .collect();
