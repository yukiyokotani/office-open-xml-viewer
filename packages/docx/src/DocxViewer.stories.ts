...
// ---------------------------------------------------------------------------
// Tracked changes & comments — synthetic fixtures with the runtime toggles.
// ---------------------------------------------------------------------------
export const TrackedChangesAndComments: Story = {
  name: 'Tracked changes & comments',
  args: { width: 700 },
  render: (args) => {
    const root = document.createElement('div');
    root.style.cssText = 'font-family:sans-serif;padding:16px;';
    ...
    for (const [value, label] of [
      ['tracked', 'Tracked changes fixture (§17.13.5)'],
      ['comments', 'Comments fixture (§17.13.4)'],
    ]) {
    ...
    const status = document.createElement('div');
    status.style.cssText = 'color:#666;font-size:13px;margin-bottom:8px;min-height:18px;';
    toolbar.append(fixtureSelect, markupBtn, commentsBtn);
    root.append(toolbar, status);

    const container = document.createElement('div');
    container.style.cssText =
      `position:relative;width:${args.width + 300}px;max-width:100%;border:1px solid #ccc;`
    ...
    const refreshButtons = () => {
      ...
      commentsBtn.textContent = `Comments: ${commentsOn ? 'on' : 'off'}`;
    };
    refreshButtons();

    async function loadFixture(): Promise<void> {
      const { generateTrackedChangesDocx, generateCommentedDocx } = await import('./conformance/generate.js');
      const bytes = fixtureSelect.value === 'comments'
      ...
      status.textContent =
        'Synthetic redistributable fixture — toggle the markup view (author-coloured '
        + 'underline/strikethrough + margin change bars) and the comment balloons.';
    }
    ...
    commentsBtn.addEventListener('click', () => {
      ...
      refreshButtons();
      void viewer?.setShowComments(commentsOn);
    });
    fixtureSelect.addEventListener('change', () => { void loadFixture(); });
    void loadFixture();
    return root;
  },
};
