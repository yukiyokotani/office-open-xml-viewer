...
//! Shared Rust document model emitted by the DOCX parser.
//!
//! This crate is an internal workspace producer contract, not a separately
//! published package or a promise to expose acquisition facts through the
//! TypeScript API. Parser-only wire members are public in Rust because the
//! parser is a separate crate and must construct the complete serialized model.
...
mod serialization_tests {
    ...
    #[test]
    fn streamed_body_unit_preserves_tag_and_payload() {
        let json = serde_json::to_string(&StreamedDocumentUnit::Body { body: vec![] }).unwrap();
        assert_eq!(json, r#"{"kind":"body","body":[]}"#);
    }

    #[test]
    fn streamed_complete_unit_keeps_section_before_body_and_private_facts() {
        let document = Document {
            document_typography_settings: Some(DocumentTypographySettingsWire {
                normal_style_font_size_pt: 11.0,
            }),
            ..Document::default()
        };
        let json = serde_json::to_string(&StreamedDocumentUnit::Complete {
            document: Box::new(document),
        })
        .unwrap();

        assert!(json.starts_with(r#"{"kind":"complete","document":{"section":"#));
        assert!(json.find(r#""section""#).unwrap() < json.find(r#""body""#).unwrap());
        assert!(json.contains(r#""__documentTypographySettings":{"normalStyleFontSizePt":11.0}"#));
    }

    #[test]
    fn default_document_omits_optional_private_sidecars() {
        let json = serde_json::to_string(&Document::default()).unwrap();
        assert!(!json.contains("__documentTypographySettings"));
    ...
    #[test]
    fn closed_acquisition_enums_preserve_wire_tags() {
        assert_eq!(
            serde_json::to_string(&AnchorAxisChoiceWire::Offset { value_pt: 3.5 }).unwrap(),
            r#"{"kind":"offset","valuePt":3.5}"#
        );
        assert_eq!(
            serde_json::to_string(&AnchorValueStatusWire::Invalid).unwrap(),
            r#""invalid""#
        );
    }
}
...
pub struct DocParagraph {
    ...
    pub run_revisions: Vec<Option<RunRevision>>,
    ...
    pub complex_field_boundaries: Vec<ComplexFieldBoundaryWire>,
    ...
    pub paragraph_typography_acquisition: Option<ParagraphTypographyWire>,
}

#[derive(Serialize, Debug, Clone, PartialEq, Eq)]
#[serde(rename_all = "camelCase")]
pub struct ComplexFieldBoundaryWire {
...
pub struct AnchorHostMetrics {
    ...
    pub anchor_occurrence_id: Option<String>,
}

#[derive(Serialize, Debug, Clone, Default)]
#[serde(rename_all = "camelCase")]
pub struct AnchorEdgesWire {
    pub top_pt: Option<f64>,
    pub top_status: AnchorValueStatusWire,
    pub right_pt: Option<f64>,
    pub right_status: AnchorValueStatusWire,
    pub bottom_pt: Option<f64>,
    pub bottom_status: AnchorValueStatusWire,
    pub left_pt: Option<f64>,
    pub left_status: AnchorValueStatusWire,
}

#[derive(Serialize, Debug, Clone, Copy, PartialEq, Eq, Default)]
#[serde(rename_all = "camelCase")]
pub enum AnchorValueStatusWire {
    #[default]
    Missing,
    Invalid,
    Valid,
...
#[derive(Serialize, Debug, Clone)]
#[serde(rename_all = "camelCase")]
pub struct AnchorAxisWire {
    pub relative_from: Option<String>,
    pub relative_from_status: AnchorValueStatusWire,
    pub choice: AnchorAxisChoiceWire,
}
...
#[derive(Serialize, Debug, Clone, Default)]
#[serde(rename_all = "camelCase")]
pub struct AnchorSimplePositionWire {
    pub enabled: Option<bool>,
    pub status: AnchorValueStatusWire,
    pub x_pt: Option<f64>,
    pub x_status: AnchorValueStatusWire,
    pub y_pt: Option<f64>,
    pub y_status: AnchorValueStatusWire,
}

#[derive(Serialize, Debug, Clone, Default)]
#[serde(rename_all = "camelCase")]
pub struct AnchorExtentWire {
    pub width_pt: Option<f64>,
    pub height_pt: Option<f64>,
    pub width_status: AnchorValueStatusWire,
    pub height_status: AnchorValueStatusWire,
}

#[derive(Serialize, Debug, Clone)]
#[serde(rename_all = "camelCase")]
pub struct AnchorRelativeSizeAxisWire {
    pub relative_from: Option<String>,
    pub relative_from_status: AnchorValueStatusWire,
    pub fraction: Option<f64>,
    pub fraction_status: AnchorValueStatusWire,
}

#[derive(Serialize, Debug, Clone, Default)]
#[serde(rename_all = "camelCase")]
pub struct AnchorRelativeSizeWire {
    pub horizontal: Option<AnchorRelativeSizeAxisWire>,
    pub vertical: Option<AnchorRelativeSizeAxisWire>,
}

#[derive(Serialize, Debug, Clone)]
#[serde(rename_all = "camelCase")]
pub struct AnchorPointWire {
    pub x: Option<i64>,
    pub y: Option<i64>,
    pub raw_x: Option<String>,
    pub raw_y: Option<String>,
}

#[derive(Serialize, Debug, Clone)]
#[serde(rename_all = "camelCase")]
pub struct AnchorPolygonWire {
    pub edited: bool,
    pub coordinate_space: AnchorPolygonSpaceWire,
    pub points: Vec<AnchorPointWire>,
    pub invalid_point_count: usize,
}

#[derive(Serialize, Debug, Clone)]
#[serde(rename_all = "camelCase")]
pub struct AnchorPolygonSpaceWire {
    pub width: u32,
    pub height: u32,
}
...
#[derive(Serialize, Debug, Clone, Default)]
#[serde(rename_all = "camelCase")]
pub struct AnchorWrapWire {
    pub kind: AnchorWrapKindWire,
    pub authored_kinds: Vec<String>,
    pub side: Option<String>,
    pub distances: AnchorEdgesWire,
    pub effect_extent: Option<AnchorEdgesWire>,
    pub polygon: Option<AnchorPolygonWire>,
}

#[derive(Serialize, Debug, Clone)]
#[serde(rename_all = "camelCase")]
pub struct AnchorBehaviorWire {
    pub behind_doc: Option<bool>,
    pub behind_doc_status: AnchorValueStatusWire,
    pub relative_height: Option<u32>,
    pub relative_height_status: AnchorValueStatusWire,
    pub locked: Option<bool>,
    pub locked_status: AnchorValueStatusWire,
    pub allow_overlap: Option<bool>,
    pub allow_overlap_status: AnchorValueStatusWire,
    pub layout_in_cell: Option<bool>,
    pub layout_in_cell_status: AnchorValueStatusWire,
}
...
#[derive(Serialize, Debug, Clone, Default)]
#[serde(rename_all = "camelCase")]
pub struct AnchorRawTransformWire {
    pub offset_x_emu: Option<f64>,
    pub offset_y_emu: Option<f64>,
    pub extent_width_emu: Option<f64>,
    pub extent_height_emu: Option<f64>,
    pub child_offset_x_emu: Option<f64>,
    pub child_offset_y_emu: Option<f64>,
    pub child_extent_width_emu: Option<f64>,
    pub child_extent_height_emu: Option<f64>,
    pub rotation_units: Option<f64>,
...
#[derive(Serialize, Debug, Clone)]
#[serde(rename_all = "camelCase")]
pub struct AnchorResolvedChildFrameWire {
    pub offset_x_pt: f64,
    pub offset_y_pt: f64,
    pub width_pt: f64,
    pub height_pt: f64,
...
#[derive(Serialize, Debug, Clone)]
#[serde(rename_all = "camelCase")]
pub struct AnchorGroupWire {
    pub child_source_id: String,
    pub source_index: usize,
    pub source_count: usize,
    pub transform_chain: Vec<AnchorRawTransformWire>,
    pub child_transform: Option<AnchorRawTransformWire>,
...
pub struct AnchorAcquisitionWire {
    ...
    pub simple_position: AnchorSimplePositionWire,
    pub horizontal: AnchorAxisWire,
    pub vertical: AnchorAxisWire,
    pub extent: AnchorExtentWire,
    pub parent_effect_extent: AnchorEdgesWire,
    pub anchor_distances: AnchorEdgesWire,
    pub relative_size: AnchorRelativeSizeWire,
...
pub struct ShapeRun {
    ...
    #[serde(skip_serializing_if = "Vec::is_empty")]
    pub text_box_content: Vec<TextBoxBlockWire>,
    ...
    pub anchor_acquisition: Option<AnchorAcquisitionWire>,
}
...
#[derive(Serialize, Debug, Clone, Default)]
#[serde(rename_all = "camelCase")]
pub struct VmlTextPathFacts {
    #[serde(skip_serializing_if = "Option::is_none")]
    pub text_path_ok: Option<bool>,
    #[serde(skip_serializing_if = "Option::is_none")]
    pub on: Option<bool>,
    #[serde(skip_serializing_if = "Option::is_none")]
...
#[serde(rename_all = "camelCase")]
pub enum TypographyValueStatusWire {
    #[default]
    Missing,
    Invalid,
    Valid,
...
#[serde(rename_all = "camelCase")]
pub struct TypographyValueWire<T> {
...
#[derive(Serialize, Debug, Clone, Default, PartialEq)]
#[serde(rename_all = "camelCase")]
pub struct CtBorderTypographyWire {
...
#[derive(Serialize, Debug, Clone, Default, PartialEq)]
#[serde(rename_all = "camelCase")]
pub struct UnderlineTypographyWire {
    pub val: TypographyValueWire<String>,
    pub color: TypographyValueWire<String>,
    pub theme_color: TypographyValueWire<String>,
    pub theme_tint: TypographyValueWire<String>,
...
#[derive(Serialize, Debug, Clone, Default, PartialEq)]
#[serde(rename_all = "camelCase")]
pub struct EastAsianLayoutTypographyWire {
...
#[derive(Serialize, Debug, Clone, Default, PartialEq)]
#[serde(rename_all = "camelCase")]
pub struct RubyGuideRunTypographyWire {
    pub text: String,
    pub font_family: Option<String>,
    pub font_size_pt: Option<f64>,
    pub bold: bool,
...
#[derive(Serialize, Debug, Clone, PartialEq)]
#[serde(rename_all = "camelCase")]
pub struct RevisionTypographyWire {
    pub kind: String,
...
#[derive(Serialize, Debug, Clone, Default, PartialEq)]
#[serde(rename_all = "camelCase")]
pub struct ParagraphTypographyBordersWire {
    #[serde(skip_serializing_if = "Option::is_none")]
...
#[derive(Serialize, Debug, Clone, Default, PartialEq)]
#[serde(rename_all = "camelCase")]
pub struct ParagraphTypographyWire {
...
pub struct RunRevision {
    ...
    #[serde(skip)]
    pub typography_id: TypographyValueWire<String>,
}
...
pub struct RubyAnnotation {
    ...
    #[serde(skip)]
    pub typography: Option<RubyTypographyWire>,
}
