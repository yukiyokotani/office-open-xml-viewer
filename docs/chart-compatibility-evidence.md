...
| Compact classic 3-D column axis with one explicit bound | 4 | With an authored maximum of 8 and omitted major unit, the observed compact vertical value axes use ticks 0, 2, 4, 6, and 8. A control with an authored display unit retains the ordinary finer density. This one-sided-bound density rule is not generalized to 2-D, horizontal, logarithmic, percent, or display-unit axes. |
