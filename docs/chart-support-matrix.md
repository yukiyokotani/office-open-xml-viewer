...
| C-SURFACE-001 | Surface/surface3D mesh, bands, camera, and authored band formatting | Yes | Yes | Yes | Partial | Both surface families retain their distinct type and render through the bounded projected source-grid mesh. Filled bands consume direct/linked/numeric structured paint in the final value-band domain and cast supported outer effects once per complete band silhouette. A wireframe uses the first Surface series outline as its mesh default, then resolves linked and numeric `dataPointWireframe` palettes by value-band index before automatic band colours. Direct `bandFmt` outlines split the source-grid edges at value-band boundaries and override that default only inside their band; first-series and band no-line remain authoritative. Width, preset/custom dash, cap, and join merge property-wise. Later-series outlines do not become mesh defaults. Compound wireframe lines remain fail-closed. Authored floor/wall thickness uses the shared projected `CT_Surface` slab geometry. Planar and positive-thickness floor/wall picture fills consume the observed stretch, projected-reference plain `stack`, bounded face-local DrawingML tile grids, and bounded `stackScale` semantics. On thick plain-stack slabs, front and lateral side faces repeat while end faces map one complete source. Picture faces independently honor `applyToFront`, `applyToSides`, and `applyToEnd`. Stretch consumes bounded signed `srcRect` source rectangles and `fillRect` destination rectangles with a positive visible intersection. Tiling is limited to the observed `pictureFormat=stretch` boundary and consumes explicit physical DPI, scale, alignment, offset, schema-defaulted or explicit flip, and a bounded signed `srcRect` inside every tile before projectively mapping the completed grid to each face. A tile/stretch-invalid `fillRect` combination remains fail-closed. Floor ignores the stack unit. Office-observed category/value major and minor gridline rules continue over their corresponding planar or positive-thickness camera-visible surface faces while retaining authored line style; horizontal Bar3D swaps the surface pairs with its axes. Inner shadow, soft-edge, reflection, and material-dependent slab shading remain unpainted on the depth-interleaved Surface mesh. |
...
The shared parser resolves all 48 built-in classic `c:style` values from
ECMA-376 §21.2.3.46 and retains the paint-bearing `CT_ChartStyle` roles defined
by MS-ODRAWXML §2.8.3.1 for both classic and ChartEx parts. The three hosts
resolve the same linked `styleN.xml` and `colorsN.xml` relationships. Direct
formatting, linked roles, and built-in roles remain separate source layers and
are composed per fill, line, text, and effect component before rendering.
Positive direct paint always wins. A direct `noFill` or no-line replaces a
present raw linked role only when that `CT_StyleEntry` carries
`allowNoFillOverride` or `allowNoLineOverride`; numeric-only built-in paint
remains directly replaceable. Authored-but-unresolved direct paint remains
fail-closed instead of exposing a lower layer. One classic-chart visibility
boundary precedes that paint cascade: `<c:ser>` / `<c:dPt>`
...
rather than by that classic mark-visibility property.
...
and fail-closed sentinels are internal and do not create a second public
precedence mode.

| ID | Role group | Status | Notes |
| --- | --- | --- | --- |
| S-STYLE-000 | Built-in classic styles 1–48 | Supported | The complete 30-role fill, line, text, and effect table is resolved from the document theme. Source-series and varying-point formatting-index domains remain distinct for pie, doughnut, bubble, of-pie, and qualifying bar groups; Excel's Style 2 repeated palette is supported through 48 points; Word and PowerPoint currently resolve the normative first six accents. Later sets use semantic automatic paint where their host transform is unverified, while independent resolved outlines are preserved. The numeric tables assign effects only to the data-mark roles consumed by the shared 2-D/3-D painters; the other numeric roles explicitly resolve to No Effect. Host-specific chart-space frames and omitted-style behavior are isolated in DOCX, XLSX, and PPTX compatibility adapters. |
| S-STYLE-001 | Shared role parsing and package wiring | Partial | Numeric roles plus linked paint recipes, fixed/relative Chart Colors indices, `NoStyle`, and bounded palette expansion are retained through DOCX, XLSX, and PPTX. `cycle` and the base-color selection of all four linear methods follow MS-ODRAWXML. The standard does not define the linear brightness endpoints or color space; a generated probe whose built-in color-style part was rewritten by Excel did not establish that missing curve, so the viewer deliberately preserves the selected authored color instead of inventing one. |
| S-STYLE-002 | `chartArea`, `plotArea`, `legend` | Partial | Positive direct paint remains authoritative. Direct `noFill`/no-line follows the modifier-gated rule above; an omitted fill or line component inherits. One bounded PowerPoint ChartEx host delta is retained: a fill-only chart-space carrier with `allowNoLineOverride` suppresses the linked chart-area outline, while an empty carrier does not. Effective numeric/linked solid, gradient, and pattern fill/outline recipes are consumed through shared frame painters across classic 2-D, optional 3-D, ChartEx, and offline Region Map paths, including host rotation and partial manual-layout dimensions. Preset and bounded custom dash, width, cap, join, and the Office-observed compound rails are supported. Explicit pen alignment and miter-limit geometry remain unpainted. |
| S-STYLE-003 | `categoryAxis`, `valueAxis`, tick labels, `seriesAxis` | Partial | Effective numeric/linked solid line and text defaults are consumed for primary, secondary, and 3-D series axes, with direct paint authoritative. Arbitrary linked gradient/pattern axis outlines, custom dash, compound rails, and non-No-Effect recipes remain unpainted. |
| S-STYLE-004 | `seriesLine`, `dropLine`, `hiLoLine`, `upBar`, `downBar` | Partial | Positive direct formatting wins; direct no-paint follows the modifier-gated rule above, and the effective numeric/linked role supplies omitted solid fill/line properties. Decoration lines retain visibility, color, width, preset dash, cap, and join. Structured decoration outlines, custom dash/compound geometry, and general linked effects remain unpainted; up/down bar mark effects are supported. |
| S-STYLE-005 | `errorBar`, `leaderLine` | Partial | Positive direct line paint wins; direct no-line follows the modifier-gated rule above. Effective numeric/linked solid color, width, preset dash, and visibility are consumed in shared 2-D and optional 3-D paths. Structured outlines, custom dash/compound geometry, and effects remain unpainted. |
| S-STYLE-006 | `trendline` | Partial | Positive direct trendline paint wins; direct no-line follows the modifier-gated rule above. Effective numeric/linked solid color, width, preset dash, and visibility are consumed by plot and legend. Structured outlines, custom dash/compound geometry, and effects remain unpainted. |
| S-STYLE-007 | `dataTable` | Partial | Positive direct table formatting wins; direct no-paint follows the modifier-gated rule above. Effective numeric/linked solid paint supplies omitted grid and frame properties. Structured grid outlines, custom dash/compound geometry, and effects remain unpainted. |
| S-STYLE-008 | `gridlineMajor`, `gridlineMinor` | Partial | Effective numeric/linked solid line fallback applies to enabled primary and secondary gridlines. Positive direct paint wins; direct no-line follows the modifier-gated rule above. Structured outlines, custom dash/compound geometry, and effects remain unpainted. |
| S-STYLE-009 | `dataLabelCallout`, `trendlineLabel` | Partial | Positive direct point, series, and trendline paint remains authoritative; direct shape no-paint follows the modifier-gated rule above, while linked roles fill omitted text, body, and shape properties in classic 2-D labels, pie/doughnut callouts, the implemented ChartEx label paths, and optional 3-D data labels. Rich runs, paragraph alignment, bounded rotation/wrapping, signed insets, anchors, manual layout, structured shape paint, and text/shape no-fill are retained on those paths. Optional 3-D trendline labels, per-glyph `eaVert`, `just`/`dist` expansion, language-specific shaping, and rich-run wrapping remain unsupported. |
| S-STYLE-010 | `dataPoint3D`, `dataPointWireframe`, `floor`, `wall`, `plotArea3D` | Partial | `dataPoint3D` direct/effective-style solid, gradient, and pattern paint is consumed by classic 3-D bar, pie, line, and area paths and their legend keys with positive direct paint winning, modifier-gated direct no-paint, and one paint resolution per datum, series, or observed incoming line segment. Filled Surface charts resolve direct `bandFmt`, subject to the same no-paint gate, over the raw linked role and then a separate numeric role in the final value-band index domain; all styles 1–48 and the count-dependent Table 5 Fade boundary are covered without aliasing source-series indices. The same resolved band paint drives faces and legend keys, and supported outer effects are emitted once per band silhouette. Bands above the observed 48-object boundary use semantic automatic paint. Picture-filled 3-D legend swatches remain fail-closed. A Surface wireframe consumes the first series outline as the mesh default, then linked-over-numeric `dataPointWireframe` paint indexed by value band, then automatic band colours; direct `bandFmt` outlines use the same positive-paint/no-line distinction per value band. Width, preset/custom dash, cap, and join merge property-wise. Later-series outlines are not promoted to mesh defaults; compound lines fail closed. Structured floor/wall and `plotArea3D` paint is consumed with direct surface formatting authoritative. Authored thickness uses the shared bounded `CT_Surface` slab projection; planar and positive-thickness floor/wall images consume stretch, projected-reference plain `stack`, bounded face-local DrawingML tile grids, and bounded `stackScale` face mappings. Thick plain-stack front/side faces repeat while end faces map one complete source. Stretch consumes bounded signed source/destination rectangles; each tile consumes its own bounded signed source rectangle. Authored category/value major and minor gridline styles continue across the measured planar and camera-visible positive-thickness slab faces, including the horizontal Bar3D axis swap. Tile/stretch-invalid destination rectangles, non-outer Surface effects, direct 3-D area point body paint, and material-dependent slab shading remain fail-closed or unpainted. |
| S-STYLE-011 | `dataPointMarker` and marker layout | Partial | Direct and linked solid, gradient, pattern, and relationship-backed picture fills plus structured marker outlines share the DrawingML paint model across implemented classic marker-bearing series and ChartEx box markers. Point and series positive paint precedes linked then numeric paint; a point/series no-paint may cross a present raw linked role only through its matching allow modifier. Source-series indices, atomic preset/custom dash choice, and authored unresolved-paint suppression are shared by 2-D and optional 3-D marker paths. Picture markers retain raster/SVG twins, source cropping, stretch, chained `alphaModFix`, and the supported two-color duotone effect. Tiling is drawn only when alignment, positive scale, positive authored DPI, and rotation semantics are explicit; omitted embedded-DPI defaults, unsupported blip effects, and unresolved relationships fail closed rather than inventing an automatic marker color. |
| S-STYLE-012 | Chart text and `title` / `axisTitle` shape components | Partial | Numeric/linked/direct text defaults are consumed with atomic text-paint precedence. Across titles, axes, legends, labels, data tables, trendline labels, and display-unit labels, an absent, empty, or metadata-only character-property carrier inherits. A carrier with substantive visual formatting owns the run and resolves an omitted `b`/`i` to regular; an explicit boolean always wins. Numeric title typography is applied only when `c:style` is present because the optional element has no schema default; the Office-observed omitted-style compatibility keeps automatic axis/mark paints but uses a regular-weight title below any direct or chart-space text formatting. For classic dark styles, Word-produced evidence covers automatic theme-`lt1` text and the direct `a:pPr/a:defRPr` title-carrier boundary across styles 41–48. Excel and PowerPoint ordinary text are bounded to the verified style 41 control; PowerPoint title carriers are likewise bounded to style 41, while Excel title carriers and PowerPoint styles 42–48 remain unclaimed. Empty rich paragraphs, sibling `c:title/c:txPr`, and unverified multi-paragraph titles do not activate the compatibility rule. Direct and linked text paint still win. Title and axis-title frame fill, outline, and effects are retained only where an existing label-box path exposes them; general classic title-frame shape painting remains unimplemented. |
| S-STYLE-013 | Arbitrary linked-role effects | Partial | Fill/line/text/effect ownership is resolved independently and authored empty, unsupported, or unreadable effects suppress lower fallbacks. Chart/plot/legend frames, implemented label boxes, data marks, markers, dataPointLine bodies, and up/down bars consume the bounded effect recipes with order-independent aggregate admission. Effects on axis rules, data-table grids, and ordinary decorations remain unpainted. This does not reduce S-STYLE-000: ECMA-376 assigns No Effect to those roles in numeric styles 1–48. |
