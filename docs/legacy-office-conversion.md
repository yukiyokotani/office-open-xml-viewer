...
| DOC-42 | Preserve table positioning at TIstd application | Native before/after controls retain anchors, positive absolute positioning and all four positive wrapping distances; the scoped reset implementation remains pending |
| DOC-43 | Preserve dimensions and style options at TIstd application | Native controls retain exact height, preferred width, autofit, the standalone bidi flag and style options. Gap controls establish only before/after equality; revision semantics and the two independent bidi flags require scoped handling |
...
| DOC-47 | Project unconditional TAPX table borders | Native inheritance and before/after-TIstd D613/D62F controls establish bounded LTR ordering and direct source-cell ownership. Model integration, retained-memory accounting and boundary coverage remain pending |
...
| DOC-61 | Preserve border origin through direct table projection | DOC-47 review found row/style fallback borders serialized as cell-owned borders, hiding the layer needed for shared-edge conflicts. A thin direct-cell versus thick style-border native control determines the required correction; use existing table/row/cell model layers, without renderer guesses |
